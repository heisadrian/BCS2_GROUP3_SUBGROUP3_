...
if (
    isset($_SESSION['expire_time'])
    && (time() - $_SESSION['expire_time']) > $timeout_duration
) {
    session_unset();
    session_destroy();
    header('Location: index.php');
...
$stmt = $conn->prepare("SELECT links FROM users WHERE user_id = ?");
...
if ($results->num_rows > 0) {
    ...
    $links = json_decode($rows['links'], true);
} else {
    $links = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (in_array($fileExtension, $allowedExtension)) {
                if ($fileError === 0) {
                    if ($fileSize < 5 * 1024 * 1024) {
                        ...
                        $newFileName = $user['fname'] . "-" . substr(uniqid(),7,12) ."-". substr(uniqid(),8,12) . "." . $fileExtension;
                        $uploadDir = "assets/images/profiles/";
                        $uploadPath = $uploadDir . $newFileName;

                        if (move_uploaded_file($fileTempName, $uploadPath)) {
                            $sql = 'UPDATE users SET username = ?, email = ?, bio = ?, gender =?, relationship = ?, profile_pic = ? WHERE user_id = ?';
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param('sssssss', $username, $email, $bio, $gender, $relationship, $newFileName, $userId);

                            if ($stmt->execute()) {
                                echo "Modified Successfully";
                            } else {
                                echo "Failed to save in database";
                            }
    ...
    if (isset($_POST['save_links'])) {
        ...
        //echo "List of links:" . $linkjson;
        $stmt = $conn->prepare('UPDATE users SET links = ?
         WHERE user_id = ?');

        $stmt->bind_param('si', $linkjson, $userId);
        $stmt->execute();
        $stmt->close();
...
<body>
            ...
            <div class="flex items-center w-full xl:px-6 px-2 max-lg:gap-10">
                ...
                <div class="2xl:w-[--w-side] lg:w-[--w-side-sm]">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <div id="logo">
                            <a href="feed.php">
                                <img src="assets/images/logo.png" alt="" class="w-20 md:block hidden dark:!hidden">
                    ...
                    <div class="flex md:gap-8 gap-4 items-center md:p-8 p-6 md:pb-4">
                        ...
                        <div class="relative md:w-20 md:h-20 w-12 h-12 shrink-0">
                            ...
                            <label for="file" class="cursor-pointer">
                                <img id="img" src="assets/images/profiles/<?php echo $user['profile_pic']; ?>"
                        ...
                        <div class="flex-1">
                            <h3 class="md:text-xl text-base font-semibold text-black dark:text-white"><?php echo htmlspecialchars($user['username']); ?></h3>
                            <p class="text-sm text-blue-600 mt-1 font-normal"><?php echo htmlspecialchars($user['email']); ?></p>
                        </div>
                    </div>
                        ...
                        <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>"
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="md:flex items-center gap-10">
                                    ...
                                    <div class="flex-1 max-md:mt-4">
                                        <input type="text" name="username"
                                            ...
                                            placeholder="Monroe" class="lg:w-1/2 w-full">
                                    </div>
                                </div>
                                ...
                                <div class="md:flex items-center gap-10">
                                    ...
                                    <div class="flex-1 max-md:mt-4">
                                        <input type="text" name="email"
                                            ...
                                            placeholder="info@mydomain.com" class="w-full">
                                    </div>
                                </div>
                                ...
                                <div class="md:flex items-start gap-10">
                                    ...
                                    <div class="flex-1 max-md:mt-4">
                                        <textarea class="w-full"
                                            name="bio" value="" rows="5"
                                            placeholder="Inter your Bio"><?php echo htmlspecialchars($user['bio']); ?></textarea>
                                    </div>
                                </div>
                                ...
                                <div class="md:flex items-center gap-10">
                                    ...
                                    <div class="flex-1 max-md:mt-4">
                                        <select name="gender" class="!border-0 !rounded-md
                                            lg:w-1/2 w-full">
                                ...
                                <div class="md:flex items-center gap-10">
                                    ...
                                    <div class="flex-1 max-md:mt-4">
                                        <select name="relationship" class="!border-0 !rounded-md
                    lg:w-1/2 w-full">
                                            <option <?php if ($user['relationship'] === "None")
                                                        echo "selected"; ?> value="None">
                                                None
                                            </option>
                                            <option <?php if ($user['relationship'] === "Single")
                                                        echo "selected"; ?> value="Single">
                                                Single
                                            </option>
                                            <option <?php if ($user['relationship'] === "In Relationship")
                                                        echo "selected"; ?> value="In Relationship">
                                                In a relationship
                                            </option>
                                            <option <?php if ($user['relationship'] === "Married")
                                                        echo "selected"; ?> value="Married">
                                                Married
                                            </option>
                                            <option <?php if ($user['relationship'] === "Engaged")
                                                        echo "selected"; ?> value="Engaged">
                                                Engaged
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="md:flex items-start gap-10 ">
                                    ...
                                    <div class="flex-1 flex items-center gap-5 max-md:mt-4">
                                        <input
                                            ...
                                            class="hidden-input"
                                            accept="image/*">
                                        <img id="ImagePreview" src="assets/images/profiles/<?php
                                                                                            echo $user['profile_pic'] ?>" alt=""
                                            class="w-10 h-10 rounded-full">
                                        <button
                                            type="button"
                                            class="change-button px-4 py-1 rounded-full bg-slate-100/60
                        border dark:bg-slate-700 dark:border-slate-600 
                        dark:text-white"
                            ...
                            <div class="flex items-center gap-4 mt-16 lg:pl-[10.5rem]">
                                <button type="submit" class="button lg:px-6 bg-secondery 
        max-md:flex-1">
                                    Cancle
                                </button>
                                <button type="submit"
                            ...
                            <div class="max-w-md mx-auto">
                                ...
                                <div class="font-normal text-gray-400">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-3 font-normal text-gray-600 dark:text-white">
                                            We may still send you
                                            important notifications about
                                            your account and content outside of you
                                            preferred notifications settings</p>
                                    </div>
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                <input type="text" name="instagram" class="w-full"
                                                    value="<?php echo $links['instagram'] ?? ""; ?>"
            ...
            max-md:flex-1">
                                        Cancle</button>
                                    <button type="submit" name="save_links"
                                        class="button lg:px-10 bg-primary text-white max-md:flex-1">
                                        Save</button>
                                </div>
    ...
    <script src="assets/js/fsimplebar.js"></script>
